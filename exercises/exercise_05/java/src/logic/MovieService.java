...
public class MovieService extends MovieServiceBase {
    ...
    public Document findMovieByTitle(String title) {
        //TODO : implement
        // find movie by title and return it
        Document searchQuery = new Document();
        searchQuery.put("title", title);
        return movies.find(searchQuery).first();
    }
    ...
    public FindIterable<Document> getBestMovies(int minVotes, double minRating, int limit) {
        //TODO : implement
        Document query = new Document("votes", new Document("$gt", minVotes)).append("rating", new Document("$gt", minRating));
        return movies.find(query).sort(Sorts.descending("votes", "rating")).limit(limit);
    }
    ...
    public FindIterable getByTweetsKeywordRegex(String keyword, int limit) {
        //TODO : implement
        FindIterable<Document> result = movies.find(new Document("tweets.text", Pattern.compile(".*" + keyword + ".*", Pattern.CASE_INSENSITIVE))).limit(limit);
        return result;
    }
